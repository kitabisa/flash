...
const Active = require('../object/memberShip/memberActive')
...
describe('simulation Payment using permata', function() {
    ...
    this.timeout( 30000 )
    it('Get Member isActive from service group id', async() => {
        const res = await Active.memberActive(global.access_Tokens1, global.serviceids1)
            if(res.status !== 200){
            ...
            global.extmemberID = res.body.data[0].external_member_id
    ...
    it('Get Membership Card from service group id', async() => {
        const res = await Card.memberCard(global.access_Tokens1, global.serviceids1)
            ...
            assert(res.status).to.equal(200)
            assert(res.body.data[0].primary_card).to.have.property("end_at").exist
            assert(res.body.data[0].primary_card).to.have.property("external_member_id").to.equal(global.extmemberID)
            assert(res.body.data[0].primary_card).to.have.property("full_name").exist
            assert(res.body.data[0].primary_card).to.have.property("member_id").exist
            assert(res.body.data[0].primary_card).to.have.property("start_at").exist
            assert(res.body.data[0].primary_card).to.have.property("status").exist
