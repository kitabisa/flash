...
describe('API Calculation Service', () => {
    describe('Get Calculate Payment', () => {
        before(async function calucated() {
            ...
            totalFee = datas.calcu.contributeFee  * persenfee
            totalFee1 = fixfee
            totalpayment = global.totalFee
            totalpayment1 = datas.calcu.contributeFee + global.totalFee1
        });
        it(`${TC_CalcuFixfee.positive.fixfee}`, async() => {
            ...
            assert(res.body.data[0]).to.have.property("total_fee").to.equal(totalFee1)
